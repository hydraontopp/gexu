...
void GameData::sendPacketToInjector(gexuDataPacket gexuDataPack) {
	if (!isInjectorConnectionActive())
		throw std::exception("gexu injector connection not active");
	if (gexuDataPack.dataArraySize >= 3000) {
		logF("Tried to send data packet with array size: %i %llX", gexuDataPack.dataArraySize, gexuDataPack.data.get());
		throw std::exception("Data packet data too big");
	}

	gexuToInjectorQueue.push(gexuDataPack);
}
void GameData::callInjectorResponseCallback(int id, std::shared_ptr<gexuDataPacket> packet) {
	if (this->injectorTogexuResponseCallbacks.find(id) == this->injectorTogexuResponseCallbacks.end()) {
		logF("No response callback for request with id=%i!", id);
		return;
	}
	this->injectorTogexuResponseCallbacks[id](packet);
	this->injectorTogexuResponseCallbacks.erase(id);
}
