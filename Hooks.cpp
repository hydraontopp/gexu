...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour gexu prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
	...
	/*
	{
		...
		if (!g_Data.isInjectorConnectionActive() && !hasSentWarning) {
			...
			if (elapsedFlot > 1.5f && !hasSentWarning) {
				hasSentWarning = true;
				auto box = g_Data.addInfoBox("Warning", "Your injector doesn't seem to connect to gexu correctly.\nYou can ignore this, but some features may not work as expected.");
				box->closeTimer = 5;
				...
				std::string text = "Download the new injector at http://gexubeta.club/";
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			...
			if (g_Data.allowWIPFeatures() && g_Data.isInjectorConnectionActive()) {
				if (HImGui.Button("Load Script Folder", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
					...
					packet.params[0] = g_Data.addInjectorResponseCallback([](std::shared_ptr<gexuDataPacket> pk) {
...
void prepCoolBean() {
	if (g_Data.getClientInstance() && g_Data.getClientInstance()->minecraftGame->getServerEntries() && *reinterpret_cast<__int64*>(g_Data.getClientInstance()->minecraftGame->getServerEntries() + 0x50)) {
...

		{
			...
			t->serverName.setText("gexu Server");
			t->serverName2.setText("gexu Server");  // This is the one actually displayed
			t->domain.setText(".gexubeta.club");
			t->serverAddress.setText("mc.gexubeta.club");
