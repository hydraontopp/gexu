...
#include "../gexu/Config/AccountInformation.h"
...
private:
	...
	std::queue<gexuDataPacket> gexuToInjectorQueue;
	std::map<int, std::function<void(std::shared_ptr<gexuDataPacket>)>> injectorTogexuResponseCallbacks;
	int lastRequestId = 0;
...
public:
	...
	void sendPacketToInjector(gexuDataPacket gexuDataPack);
	inline int addInjectorResponseCallback(std::function<void(std::shared_ptr<gexuDataPacket>)> callback) {
		lastRequestId++;
		this->injectorTogexuResponseCallbacks[lastRequestId] = callback;
		return lastRequestId;
	}
	void callInjectorResponseCallback(int id, std::shared_ptr<gexuDataPacket> packet);
	...
	inline void setInjectorConnectionActive(bool isActive) {
		if (injectorConnectionActive && !isActive) {
			std::queue<gexuDataPacket> empty;
			gexuToInjectorQueue.swap(empty);
		}
		injectorConnectionActive = isActive;
	};
	inline bool isPacketToInjectorQueueEmpty() { return gexuToInjectorQueue.empty(); };
	inline gexuDataPacket getPacketToInjector() {
		if (isPacketToInjectorQueueEmpty())
			throw std::exception("Packet send queue is empty");
		gexuDataPacket pk = gexuToInjectorQueue.front();
		gexuToInjectorQueue.pop();
		return pk;
	};
